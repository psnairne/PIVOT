...
impl HgvsVariant {
    ...
    pub fn validate_against_gene(&self, gene: &str) -> Result<(), PivotError> {
        if is_hgnc_id(gene) && gene == self.hgnc_id {
            Ok(())
        } else if is_hgnc_id(gene) && gene != self.hgnc_id {
            Err(PivotError::IncorrectGeneData(format!(
                "Provided HGNC ID {gene} does not match with HGVS variant {self:?}"
            )))
        } else if !is_hgnc_id(gene) && gene == self.symbol {
            Ok(())
        } else {
            Err(PivotError::IncorrectGeneData(format!(
                "Provided gene symbol {gene} does not match with HGVS variant {self:?}"
            )))
        }
    }
